...
function processDir(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // This is a bit harder with regex, let's just append inside cn() if possible, but let's see if we can do it simpler.
      // A better way is to wrap existing cn() or add className if missing.
