  ...
  const schema = useMemo(() => {
    return z.object({
      email: z
        .string()
        .trim()
        .email(t("auth.validation.emailRequired") || "Please enter a valid email address."),
    });
  }, [t]);
          ...
          <div className="glass-panel rounded-3xl border border-white/50 bg-white/65 p-7 shadow-[0_20px_70px_rgba(15,23,42,0.12)] backdrop-blur-3xl">
            {sent ? (
              <div className="space-y-4 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-800">{t("auth.checkInboxTitle") || "Check Your Inbox"}</h3>
                <p className="text-sm font-medium leading-relaxed text-slate-600">
                  {t("auth.checkInboxDesc") || "We have sent password recovery instructions to your registered email address."}
                </p>
                <Link
                  to="/login"
              ...
              <form onSubmit={handleSubmit(submitReset)} noValidate className="space-y-5">
                <div className="mb-4 border-b border-slate-200/70 pb-4">
                  ...
                  <p className="text-xs font-semibold text-slate-500">
                    {t("auth.recoveryDetailsDesc") || "Enter your email below to receive a reset link."}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="email"
                    {...register("email")}
                    placeholder={t("auth.emailPlaceholder") || "e.g. yourname@example.com"}
