...
const roleTabs: Array<{
...
}> = [
  {
    ...
    subtext: "Access your customer account to book artists & track events",
  },
  {
    id: "artist",
    label: "Artist Login",
    ...
    subtext: "Access your artist dashboard to manage bookings & profile",
  },
];
...
export default function ArtistLogin() {
  ...
  const isUserInitial =
    location.pathname.includes("user") || searchParams.get("role") === "user";

  const [activeRole, setActiveRole] = useState<LoginRole>(
    isUserInitial ? "user" : "artist"
  );
  ...
  useEffect(() => {
    ...
    if (roleParam === "user" || roleParam === "artist") {
  ...
  const submitLogin = async (data: LoginValues) => {
    ...
    try {
      ...
      const sdkEmail = rawUsername.includes("@")
        ? rawUsername
        : `${rawUsername}@mykalakar.app`;

      await signInWithEmailAndPassword(auth, sdkEmail, data.password);

      if (
        rawUsername === "vortex" ||
        rawUsername === "admin12@gmail.com" ||
        rawUsername === "admin"
      ) {
        ...
        window.dispatchEvent(new Event("MYKALAKAR_MASTER_ADMIN_CHANGED"));
        const redirectPath =
          (location.state as any)?.from || (location.state as any)?.returnTo;
        if (redirectPath) {
          navigate(redirectPath, { replace: true, state: location.state });
        } else {
          const defaultPath =
            activeRole === "artist" ? "/artist/dashboard" : "/";
      ...
      <div className="auth-stage mx-auto grid w-full max-w-6xl items-center gap-6 py-8 lg:grid-cols-[0.95fr_1fr]">
        <div className="auth-visual-panel hidden min-h-[620px] overflow-hidden rounded-lg border border-white/10 bg-white/[0.055] p-8 shadow-2xl backdrop-blur-2xl lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="auth-visual-stack grid gap-3">
            {[
              ...
              <div
                key={item}
                className="rounded-lg border border-white/10 bg-white/[0.07] p-4 text-sm font-extrabold text-white/82 backdrop-blur-2xl"
              >
                {item}
              </div>
            ))}
        ...
        >
          <div className="mb-6 text-center">
            ...
            <h1 className="font-display text-3xl sm:text-4xl font-black tracking-tight text-[#1A1A1A]">
              {t("auth.loginTitle")}
            </h1>
          ...
          <form
            ...
            className="glass-panel rounded-3xl border border-white/50 bg-white/75 p-6 sm:p-7 shadow-[0_20px_70px_rgba(15,23,42,0.12)] backdrop-blur-3xl"
            noValidate
          >
            {/* ─── Role Switcher Tabs (User vs Artist) ─── */}
            <div className="mb-6 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100/90 p-1.5 border border-slate-200/80">
              {roleTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 rounded-xl py-2.5 px-3 text-xs font-black transition-all duration-200 ${
                      isActive
                  ...
                  >
                    ...
                    />
                    <span>{tab.id === "user" ? "User" : "Artist"}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="mb-6 flex items-center gap-3 border-b border-slate-200/70 pb-4">
              <div
                className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-r ${activeTab.color} text-white shadow-md`}
              >
                {activeRole === "user" ? (
              ...
              <div>
                <h2 className="font-display text-lg font-extrabold text-[#2E3A47]">
                  {activeRole === "user"
                    ? "User Account Login"
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className={`input-glass w-full px-4 py-3 pl-4 text-sm ${
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`input-glass w-full px-4 py-3 pr-11 text-sm ${
              ...
              <button
                ...
                className={`flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r ${
                  activeTab.color
                } text-sm font-black uppercase tracking-widest text-white shadow-lg transition ${
                  !isValid || submitting
            ...
            <div className="mt-4 pt-3 border-t border-slate-200/60 text-center">
              <Link
                to="/admin/login"
                className="inline-flex items-center gap-1.5 text-xs font-black text-slate-600 hover:text-orange-600 transition"
              >
                <ShieldCheck className="h-3.5 w-3.5 text-orange-500" /> Admin
                Portal Login →
              </Link>
            </div>
          </form>
