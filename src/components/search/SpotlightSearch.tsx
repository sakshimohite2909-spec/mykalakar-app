...
export function SpotlightSearch({ className = "" }: { className?: string }) {
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <Search className="pointer-events-none absolute left-3.5 sm:left-4 top-1/2 z-10 h-3.5 w-3.5 sm:h-4 sm:w-4 -translate-y-1/2 text-stone-400" />
      <input
        value={query}
        ...
        className="h-10 sm:h-12 md:h-14 w-full rounded-xl sm:rounded-2xl border border-stone-200 bg-white/95 sm:bg-stone-50 pl-9 sm:pl-11 pr-3 sm:pr-4 text-xs sm:text-sm font-extrabold text-stone-950 outline-none transition placeholder:text-stone-400 placeholder:font-bold focus:border-orange-400 focus:bg-white focus:ring-2 sm:focus:ring-4 focus:ring-orange-500/10 shadow-sm"
      />
